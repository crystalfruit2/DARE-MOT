# DARE-MOT -- CA motion model: std_weight_acceleration sweep (2026-07-31).
# ============================================================================================
# The 07-29 CA baseline win (-29 IDSw / +1.4% IDF1 / +1.1% MOTA macro vs CV headline) used
# std_weight_acceleration = 1/160, set purely by analogy to the existing velocity-noise weight
# and never tuned. This sweeps it on a log-ish grid to see whether a tuned value beats the
# untuned reference. Same config as mc_ca_baseline otherwise (two-frame gate OFF, full headline
# IoU-gate + appearance config), seed=0, val7.
#
# Grid: 1/640, 1/320, 1/80, 1/40, 1/20 (1/160 is the existing mc_ca_baseline run -- not re-run).
#
# Usage: _run_ca_accel_sweep.ps1 [ckpt]
# ============================================================================================
...
function Run-CaAccel($accelNoise, $expn) {
  Clear-DareEnv
  ...
  $env:DARE_KF_ACCEL_NOISE="$accelNoise"
  ...
  Write-Output "########## RUN $expn (CA, accel_noise=$accelNoise) ##########"
...
$t0 = Get-Date
Write-Output "########## CA ACCEL-NOISE SWEEP START (detector = $ckpt) ##########"
Run-CaAccel "0.0015625" "mc_ca_accel_640"
Run-CaAccel "0.003125"  "mc_ca_accel_320"
Run-CaAccel "0.0125"    "mc_ca_accel_80"
Run-CaAccel "0.025"     "mc_ca_accel_40"
Run-CaAccel "0.05"      "mc_ca_accel_20"
